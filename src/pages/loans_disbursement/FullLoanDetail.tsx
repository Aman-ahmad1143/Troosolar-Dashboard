...
export interface LoanDetail {
  ...
  interestRate: string;
  sendStatus: "Pending" | "Completed" | "Rejected";
  sendDate: string;
  approvalStatus: "Pending" | "Completed" | "Rejected";
  approvalDate: string;
  disbursementStatus: "Pending" | "Completed" | "Rejected";
...
const FullLoanDetail: React.FC<FullLoanDetailProps> = ({ isOpen, onClose, loanId, onDisbursementUpdate }) => {
  ...
  const handleDisburse = (loanId: string, status: string) => {
    if (selectedLoan) {
      const updatedLoan: LoanDetail = {
        ...selectedLoan,
        disbursementStatus: status as "Pending" | "Completed" | "Rejected",
        disbursementDate: status === 'Completed' ? new Date().toLocaleDateString() : 'Not yet disbursed'
      };
      ...
      <div className="relative bg-white w-full h-full sm:w-[675px] sm:h-[900px] rounded-t-xl sm:rounded-xl shadow-lg overflow-y-auto z-10">
        ...
        {activeTab === 'loan' && (
          <div className="px-5">
            ...
            <button 
              className={`w-full py-3 rounded-md font-medium my-5 transition-colors ${
                selectedLoan.disbursementStatus === 'Completed' 
                  ? 'bg-gray-400 text-gray-600 cursor-not-allowed' 
                  : 'bg-blue-600 hover:bg-blue-700 text-white'
              }`}
              onClick={() => selectedLoan.disbursementStatus !== 'Completed' && setShowDisburseModal(true)}
